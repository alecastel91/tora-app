...
**Pricing** is multi-currency (EUR · JPY · USD) since 2026-09-23 — the single source of truth is [tora-backend/src/config/pricing.js](../tora-backend/src/config/pricing.js) (table + routing rules in tora-backend/CLAUDE.md). Frontend mirror: [`src/utils/money.js`](src/utils/money.js) — `MEMBER_PRICES`, `AGENT_BANDS`, `EXTRA_PRICES`, `formatMoney(amount, currency)` (¥3,480 / €19.90 / $22.90). The Premium page reads the account currency from `GET /billing/status` (`currency`, fixed once a Stripe customer exists, previewed from the verified country before that) and every quote from the server carries `amountDue` + `currency`. Never hard-code a currency symbol or amount in a screen or a translation.
